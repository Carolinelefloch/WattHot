...
Assumption:
	1. For Tier Type Rate Calculation. Lowest Tier(Tier 1) for all 30days
	2. For TOU and Tier Type(TOU 1,1,0 type) Rate Calculation. Tier Arrangement based on the customer's cost
	3. Monthly Allowance retrieve directly from the Tariff book database
			No.Tier 1=30 cost <100
			No.Tier 1=20;No.Tier 2=10 if cost>100 and cost<200
			No.Tier 1=15;No.Tier 2=15 if cost>200
	4.Assuming the season of the customer cost input is average of Summer Cost and Winter Cost to Estimate the Monthly electricity consumption
'''
...
class Cost:
	...
	def Get_EV_Def_Cost(self,Charging_Outside,Utility_Name,Rate_Name,EV_Load,Ls_App,No_EV=5,Cost=0):
		...
		Tier_Type=self.c1.execute(command,Input).fetchall()[0]
		Tier_Type=Tier_Type[0]     					
		if Tier_Type==0:
			Tier=[];[Tier.append(0) for i in range(30)]							#Assume Tier=1 for all 30 days
		else:
			if Cost<=70:
				...
				Tier=[];[Tier.append(0) for i in range(10)];[Tier.append(1) for i in range(20)];
		print Tier
		EV_Cost=0
		for index in range(30):
			T=Tier[index];W=self.Week_day[index]
			Input=(Utility_Name,Rate_Name,T,W,'summer')
			command=(
				'''
			...
			EV_Cost+=sum(np.multiply(Adj_EV_Load,Rate_Vector))/float(4)
	...
	#Put three input(number of day/night/room) to a List
		...
		if Cost==0:
			...
			Cost-=Def_EV_Cost
			print 'Cost w/o EV and Def Load:   '+str(Cost)
			...
			##############################################
				...
				if TOU_Type==(1,0,0) or TOU_Type==(1,0,1):
					...
					print 'Normalized Demand :'+str(Cust_Demand)
					Demand_Cost=Cust_Demand*Demand_Rate
					print 'Demand Cost is'+str(Demand_Cost)
					print 'Normalized Demand Cost:'+str(Demand_Cost)
					...
					print 'TOU Cost Summer:'+ str(TOU_Cost_Summer)
					print 'TOU Cost Winter:'+ str(TOU_Cost_Winter)
					TOU_Cost=(TOU_Cost_Winter+TOU_Cost_Summer)/2
					return round(Cost/(TOU_Cost+Demand_Cost),2)
				elif TOU_Type==(1,1,0):
					Cust_Profile=copy.copy(self.Matching(Input_List))
					#print 'sum of profile='+str(sum(Cust_Profile)/4)
					'''
					No.Tier 1=30 cost <100
					No.Tier 1=20;No.Tier 2=10 if cost>100 and cost<200
					No.Tier 1=15;No.Tier 2=15 if cost>200
					...
					for S in Season:
						for index in range(30):
							...
							if S=='summer':
								...
								TOU_Cost_Winter+=sum(np.multiply(Cust_Profile,Rate_Vector))/float(4)
					print 'TOU Cost Summer:'+ str(Cost/TOU_Cost_Summer)
					print 'TOU Cost Winter:'+ str(Cost/TOU_Cost_Winter)
					TOU_Cost=(TOU_Cost_Winter+TOU_Cost_Summer)/2
					return round(Cost/TOU_Cost,2)
	...
	def Get_Cost(self,Utility_Name,Household_Total,Household, Def_Load,EV_Load,Charging_Outside,No_EV=5,No_Def=2):
		...
		print 'Max demand is:  '+str(Max_Demand)
