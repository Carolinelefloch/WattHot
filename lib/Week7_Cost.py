...
Assumption:
	1. For Tier and TOU and Tier Type(TOU 1,1,0 type) Rate Calculation. Tier Arrangement based on the customer's  
       cost       	
...
class Cost:
	...
	def Get_EV_Def_Cost(self,Charging_Outside,Utility_Name,Rate_Name,EV_Load,Ls_App,No_EV=5,Cost=0):
		...
		if Allowance !='Na':
			print 'EV Allowance:'+str(Allowance)
			...
			print 'EV Allowance Cost:'+str(Allowance_Cost)
	...
	#Put three input(number of day/night/room) to a List
		...
		if Cost==0:
			...
			if Cust_Rate_type=='TIER':
				...
				print Tier_Tariff
				Tier_Tariff=json.loads(Tier_Tariff)
				print 'The Tier of Fixed Tier plan is:'+str(Tier_Tariff)
				'''
				Initial the results
	...
	def Get_Cost(self,Utility_Name,Household_Total,Household, Def_Load,EV_Load,Charging_Outside,No_EV=5,No_Def=2):
		...
		for P in range(No_Rate):
			...
			#if P is 0:
			print 'Revised'+Rate_Name+str(Cust_Tier)
			for S in Season:
				Max_Demand=0
				for day in range(30):
